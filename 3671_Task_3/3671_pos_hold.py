...
class Edrone():
	...
	def __init__(self):
		...
		self.error=[0,0,0]  #to store error for [pitch, roll, throttle, yaw]
		self.prev_error = [0,0,0]  #to store previous error for [pitch, roll, throttle, yaw]
		...
		self.iterm_error=[0,0,0]	#stores sum of error during entire time of pid tuning
		
		#to limit the values of each axis

		self.max_values = [1680,1680,1680]		 
	...
	def whycon_callback(self,msg):
		...
		print("self.drone_position is",self.drone_position)
	...
	#	8. Add error_sum
		...
		if(abs(self.output_pitch)>10):
			if(self.output_pitch<-10):
				self.output_pitch = -10
				print("self.output_pitch in -ve",self.output_pitch)
			elif(self.output_pitch>10):
				self.output_pitch = 10
				print("self.output_pitch in +ve",self.output_pitch)
		...
		if(abs(self.output_roll)>10):
			if(self.output_roll<-10):
				self.output_roll = -10
				print("self.output_roll in -ve",self.output_roll)
			elif(self.output_roll>10):
				self.output_roll = 10
				print("self.output_roll in +ve",self.output_roll)
		...
		print("throtle",self.cmd.rcThrottle)
...
if __name__ == '__main__':
	...
	while not rospy.is_shutdown():
		...
		e_drone.pid()		
		e_drone.rate.sleep()	#sampling time is 0.06sec
e_drone.disarm()
