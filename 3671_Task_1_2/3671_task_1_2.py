...
class Edrone():
	...
	def __init__(self):
		...
		self.Kp = [10.76, 6.596, 21.54, 0]
		self.Ki = [0.002, 0, 0, 0]
		self.Kd = [131.6, 24.6, 519.0, 0]
		self.path_points = []
		...
		self.sample_time = 0.082 # in seconds
	...
	#function to store the recieved path points or coordinates of path computed in list (self.path_points)
	def path_point_callback(self,msg):
		for a in range(len(msg.poses)):
			self.path_points.append([msg.poses[a].position.x,msg.poses[a].position.y,msg.poses[a].position.z])
		

	#function to send request to vrep to compute path
	...
	#function to follow the path points or coordinates of the path computed by vrep 
	def follow_path_points(self):
		for x in self.path_points:
			...
			#loop continues until the error between drone current position and current set point is greater than 0.5
			while (abs(self.error[0])>0.5 or abs(self.error[1])>0.5 or abs(self.error[2])>0.5 or abs(self.error[3])>0.5):		
				self.pid()
	...
	#------------------------------------------------------------------------------------------------------------------------
		...
		self.command_pub.publish(self.cmd)
		rospy.sleep(self.sample_time)	#sampling time is 0.082sec
...
if __name__ == '__main__':
	...
	e_drone = Edrone()
	publish_path = 1		#counts the path number
	dummy_target='blue'		#tells the current goal point 	


	while not rospy.is_shutdown():

		e_drone.zero_line_pub.publish(0) 	#publishing "0" to /zero_line topic 
		e_drone.pid()				#calls the function pid() to apply pid algorithm to reach current setpoint 
	

		# check whether the drone reached to initial_waypoint,goal_1,goal_2 
		if (abs(e_drone.error[0])<0.2 and abs(e_drone.error[1])<0.2 and abs(e_drone.error[2])<0.2 and abs(e_drone.error[3])<0.2):

			
			if(dummy_target=='blue' and publish_path==1):

				e_drone.send_path_req(publish_path)   #send request to vrep to publish the path from initial_waypoint to goal_1 
				publish_path=2
				print('next target')
			elif(dummy_target=='red' and publish_path==2):

				e_drone.send_path_req(publish_path)         #send request to vrep to publish the path from goal_1 to goal_2
				publish_path=3
				print('next target')
			elif(dummy_target=='green' and publish_path==3):

				e_drone.send_path_req(publish_path)    #send request to vrep to publish the path from goal_2 to initial_waypoint
				publish_path=4
				print('next target')
			elif(dummy_target=='blue' and publish_path==4):

				e_drone.disarm()			#disarm the drone or lands the drone
			
		elif(len(e_drone.path_points)>0 and publish_path==2):

			e_drone.follow_path_points()		#calls the function to follow the path computed from initial_waypoint to goal_1 
			dummy_target='red'			
			
		elif(len(e_drone.path_points)>0 and publish_path==3):

			e_drone.follow_path_points()		#calls the function to follow the path computed from goal_1 to goal_2
			dummy_target='green'	

		elif(len(e_drone.path_points)>0 and publish_path==4):

			e_drone.follow_path_points()	        #calls the function to follow the path computed from goal_2 to initial_waypoint
			dummy_target='blue'
			e_drone.setpoint = [5.68,-1.91,33.40,0]
